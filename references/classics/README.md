...
> **覆盖状态（已补全）**：v2.2.0 起 B/C 已由节选版升级为完整版，历史“缺失章节清单”（B 格局论核心章节、C 乙木/丁火/戊土/己土/庚金/辛金/癸水调候章节）已全部补齐。仅 `A_滴天髓.txt` 为节选（上篇通神论、下篇六亲论），暂无已知缺口；若需更完整可自行补充。
...
## Regenerate（旧版，已不再作为主来源）

> 早期版本通过 `scripts/extract_classics_text.py` 从 PDF 抽取文本（含 `=== PAGE N ===` 标记的节选版）。
> **v2.2.0 起 B/C 已内置完整版 txt，不再依赖 PDF 抽取**；该脚本仅保留用于历史回溯或自行替换 A 版本文本，不再需要执行。
